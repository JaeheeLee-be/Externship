...
class PostCategoryListTest(APITestCase):
    ...
    def test_list_categories_returns_200(self) -> None:
        PostCategory.objects.create(name="자유게시판")
    ...
    def test_list_categories_response_shape(self) -> None:
        category = PostCategory.objects.create(name="자유게시판")

        response = self.client.get(self.url)

        self.assertEqual(len(response.data), 1)
        self.assertEqual(response.data[0], {"id": category.id, "name": "자유게시판"})

    def test_list_categories_returns_all(self) -> None:
        names = ["자유게시판", "코스게시판", "기수게시판"]
        for name in names:
