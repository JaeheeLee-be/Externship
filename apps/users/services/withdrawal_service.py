...
def restore_user_by_token(email_token: str) -> None:
    ...
    :raises ValidationError: 토큰이 없거나 purpose가 recovery가 아닌 경우
    :raises NotFound: 해당 이메일의 탈퇴 계정이 존재하지 않는 경우
    """
    ...
    if not cached or cached.get("purpose") != AuthPurpose.RECOVERY.value:
        raise ValidationError("유효하지 않은 복구 토큰입니다.")
